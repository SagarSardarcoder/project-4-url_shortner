...
const posturl = async function (req, res) {
  try {
    ...
    if (!isValidReqBody(data))
      return res
        .status(400)
        .send({ status: false, message: "plese fill some data" });
    let { longUrl } = data;
    //-------------longUrl validation------------//
    if (!isValid(longUrl))
      return res
        .status(400)
        .send({ status: false, message: "plese enter longUrl" });
    if (!validator.isUri(longUrl.trim()))
      return res
        .status(400)
        .send({ status: false, message: "please provide a valid longUrl" });
    let findUrl = await urlModel
      .findOne({ longUrl })
      .select({ _id: 1, longUrl: 1, shortUrl: 1, urlCode: 1 });
    if (findUrl)
      return res
        .status(400)
        .send({
          status: true,
          message: `url is already shorten with this long url`,
          data: findUrl,
        });
    //-------generate shortId---------------//
    ...
    return res
      ...
      .send({
        status: true,
        message: `Succesfully url is shortend`,
        data: urlRes,
      });
  } catch (err) {
    ...
    if (parseLongUrl) {
      ...
      if (!urlData)
        return res
          ...
          .send({
            status: false,
            message: `no url found with this code ${Params}`,
          });
